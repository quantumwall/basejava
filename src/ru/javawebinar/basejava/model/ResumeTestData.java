...
public class ResumeTestData {
    ...
    public static Resume getResume(String uuid, String fullName) {
...

        {
            ...
            var companyPeriod1 = new Period("Автор проекта", "Создание, организация и проведение Java онлайн проектов и стажировок.", YearMonth.of(2013, Month.OCTOBER), null);
        ...
        }
        {
            ...
            var periods = new ArrayList<Period>();
            var companyPeriod1 = new Period("Старший разработчик (backend)", "Проектирование и разработка онлайн платформы управления проектами Wrike (Java 8 API, Maven, Spring, MyBatis, Guava, Vaadin, PostgreSQL, Redis). Двухфакторная аутентификация, авторизация по OAuth1, OAuth2, JWT SSO.", YearMonth.of(2014, Month.OCTOBER), YearMonth.of(2016, Month.JANUARY));
            periods.add(companyPeriod1);
            var company = new Company(link, periods);
            jobsList.add(company);
        }
        {
            ...
            var periods = new ArrayList<Period>();
            var companyPeriod1 = new Period("Java архитектор", "Организация процесса разработки системы ERP для разных окружений: релизная политика, версионирование, ведение CI (Jenkins), миграция базы (кастомизация Flyway), конфигурирование системы (pgBoucer, Nginx), AAA via SSO. Архитектура БД и серверной части системы. Разработка интергационных сервисов: CMIS, BPMN2, 1C (WebServices), сервисов общего назначения (почта, экспорт в pdf, doc, html). Интеграция Alfresco JLAN для online редактирование из браузера документов MS Office. Maven + plugin development, Ant, Apache Commons, Spring security, Spring MVC, Tomcat,WSO2, xcmis, OpenCmis, Bonita, Python scripting, Unix shell remote scripting via ssh tunnels, PL/Python", YearMonth.of(2012, Month.APRIL), YearMonth.of(2014, Month.OCTOBER));
            periods.add(companyPeriod1);
            var company = new Company(link, periods);
            jobsList.add(company);
        }
        ...
        }
        {
            ...
            var periods = new ArrayList<Period>();
            var companyPeriod1 = new Period("Ведущий специалист", "Дизайн и имплементация Java EE фреймворка для отдела \"Платежные Системы\" (GlassFish v2.1, v3, OC4J, EJB3, JAX-WS RI 2.1, Servlet 2.4, JSP, JMX, JMS, Maven2). Реализация администрирования, статистики и мониторинга фреймворка. Разработка online JMX клиента (Python/ Jython, Django, ExtJS)", YearMonth.of(2008, Month.JUNE), YearMonth.of(2010, Month.DECEMBER));
            periods.add(companyPeriod1);
            var company = new Company(link, periods);
            jobsList.add(company);
        }
        {
            ...
            var periods = new ArrayList<Period>();
            var companyPeriod1 = new Period("Разработчик ПО", "Реализация клиентской (Eclipse RCP) и серверной (JBoss 4.2, Hibernate 3.0, Tomcat, JMS) частей кластерного J2EE приложения (OLAP, Data mining).", YearMonth.of(2007, Month.MARCH), YearMonth.of(2008, Month.JUNE));
            periods.add(companyPeriod1);
            var company = new Company(link, periods);
            jobsList.add(company);
        }
        {
            ...
            var periods = new ArrayList<Period>();
            var companyPeriod1 = new Period("Разработчик ПО", "Разработка информационной модели, проектирование интерфейсов, реализация и отладка ПО на мобильной IN платформе Siemens @vantage (Java, Unix).", YearMonth.of(2005, Month.JANUARY), YearMonth.of(2007, Month.JULY));
            periods.add(companyPeriod1);
            var company = new Company(link, periods);
            jobsList.add(company);
        }
        {
            ...
            var periods = new ArrayList<Period>();
            var companyPeriod1 = new Period("Инженер по аппаратному и программному тестированию", "Тестирование, отладка, внедрение ПО цифровой телефонной станции Alcatel 1000 S12 (CHILL, ASM).", YearMonth.of(1997, Month.SEPTEMBER), YearMonth.of(2005, Month.JANUARY));
            periods.add(companyPeriod1);
            var company = new Company(link, periods);
            jobsList.add(company);
        }
...

        {
            ...
            var periods = new ArrayList<Period>();
            var companyPeriod1 = new Period("Functional Programming Principles in Scala' by Martin Odersky", null, YearMonth.of(2013, Month.MARCH), YearMonth.of(2013, Month.MAY));
            periods.add(companyPeriod1);
            var company = new Company(link, periods);
        ...
        }
        {
            ...
            var periods = new ArrayList<Period>();
            var companyPeriod1 = new Period("Курс 'Объектно-ориентированный анализ ИС. Концептуальное моделирование на UML.'", null, YearMonth.of(2011, Month.MARCH), YearMonth.of(2011, Month.APRIL));
            periods.add(companyPeriod1);
            var company = new Company(link, periods);
            educationList.add(company);
        }
        {
            ...
            var periods = new ArrayList<Period>();
            var companyPeriod1 = new Period("3 месяца обучения мобильным IN сетям (Берлин)", null, YearMonth.of(2005, Month.JANUARY), YearMonth.of(2005, Month.MARCH));
            periods.add(companyPeriod1);
            var company = new Company(link, periods);
            educationList.add(company);
        }
        {
            var link = new Link("Alcatel", "http://www.alcatel.ru/");
            var periods = new ArrayList<Period>();
            var companyPeriod1 = new Period("3 месяца обучения мобильным IN сетям (Берлин)", null, YearMonth.of(1997, Month.SEPTEMBER), YearMonth.of(1998, Month.MARCH));
            periods.add(companyPeriod1);
            var company = new Company(link, periods);
            educationList.add(company);
        }
        {
            ...
            var periods = new ArrayList<Period>();
            var companyPeriod1 = new Period("Аспирантура (программист С, С++)", null, YearMonth.of(1993, Month.SEPTEMBER), YearMonth.of(1996, Month.JULY));
            var companyPeriod2 = new Period("Инженер (программист Fortran, C)", null, YearMonth.of(1987, Month.SEPTEMBER), YearMonth.of(1993, Month.JULY));
            periods.add(companyPeriod1);
        ...
        }
        {
            ...
            var periods = new ArrayList<Period>();
            var companyPeriod1 = new Period("Закончил с отличием", null, YearMonth.of(1984, Month.SEPTEMBER), YearMonth.of(1987, Month.JULY));
            periods.add(companyPeriod1);
            var company = new Company(link, periods);
            educationList.add(company);
        }
